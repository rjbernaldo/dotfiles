...
function* exchange_link(states) {
	...
	const aliasStorage = this.getExchangeStorageAlias()
	const stored = (yield* StorageInternal("sync").getStorageValue("exchanges", aliasStorage)) || {}
	const hasCredentials = Object.keys(stored).length > 0 // has account(s)
...
function faq() {
	window.open("https://use.autoview.with.pink/hc/en-us/sections/115000647347-FAQ");
}
...
function* init() {
	...
	if (hasSubscription) {
		const elements = document.getElementsByClassName("ad");
