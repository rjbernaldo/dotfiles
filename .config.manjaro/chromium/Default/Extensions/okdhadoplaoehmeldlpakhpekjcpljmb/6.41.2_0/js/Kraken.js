...
function Kraken() {
	...
	function* account(symbol) {
		const balances = yield* post.call(this, "/Balance")
		...
		try {
			...
			if (response.error && response.error.length) {
				...
				if (error[1] === "E") {
					...
					Verbose().warn(this.getExchangeName(), error[2])
				}
			}
	...
	function* ordersCancelAll(Command) {
		...
		orders = orders.filter((order) => {
			if (Command.b) {
				if (order.descr.type !== (Command.isBid ? "buy" : "sell")) {
		...
		for (let i = 0; i < orders.length; i++) {
			...
			if (Command.d) {
				Verbose().info("Order", order.vol, "@", order.descr.price, "would be cancelled")
			} else {
	...
	function* time() {
		const response = yield* this.getRequest(state.endpoint + "/" + state.version + "/public/Time", null, null, "json")
		const serverTime = response.result.unixtime * 1000
	...
	function* trade(Command) {
		...
		let available
		if (Command.isMarginTrading && Command.y === "equity") {
			available = yield* accountEquity.call(this, currency)
		} else {
			available = yield* account.call(this, currency)
		}
		...
		if ((Command.hasOwnProperty("fsl") && Command.hasOwnProperty("ftp")) || (Command.hasOwnProperty("sl") && Command.hasOwnProperty("tp"))) {
			...
			params.price = sl.resolve(market.pair_decimals) // stop loss price
			params.price2 = tp.resolve(market.pair_decimals) // take profit price
		} else if (Command.hasOwnProperty("fsl") || Command.hasOwnProperty("sl")) {
			const sl = Command.fsl ? Command.fsl : Command.sl.relative(first)
			side = Command.isBid ? "sell" : "buy"
			if (Command.t === "market") {
				...
				params.price = sl.resolve(market.pair_decimals) // stop loss trigger price
				params.price2 = price // triggered limit price
			}
		} else if (Command.hasOwnProperty("ftp") || Command.hasOwnProperty("tp")) {
			const tp = Command.ftp ? Command.ftp : Command.tp.relative(first)
			side = Command.isBid ? "sell" : "buy"
			if (Command.t === "market") {
				params.ordertype = "take-profit"
				params.price = tp.resolve(market.pair_decimals) // take profit price
			} else {
