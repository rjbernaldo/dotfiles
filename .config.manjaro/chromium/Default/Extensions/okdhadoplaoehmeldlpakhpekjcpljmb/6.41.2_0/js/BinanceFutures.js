"use strict"
window.BINANCE_FUTURES_SYMBOLS = {}
window.BINANCE_FUTURES_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function BinanceFutures() {
	...
	function* symbolInfo(symbol) {
		...
		if (window.BINANCE_FUTURES_SYMBOLS_UPDATED < expired) {
			...
			response.symbols.forEach((info) => {
				...
				window.BINANCE_FUTURES_SYMBOLS[info.symbol] = info
				window.BINANCE_FUTURES_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (symbol) {
			if (!window.BINANCE_FUTURES_SYMBOLS.hasOwnProperty(symbol)) {
				throw new Error("Unknown market symbol: " + symbol)
			}
			return window.BINANCE_FUTURES_SYMBOLS[symbol]
		}

		return window.BINANCE_FUTURES_SYMBOLS
	}
	...
	function* time() {
		...
		if (offset > 1) {
			Verbose().error("Your computer's clock is too far into the future for Binance Futures (> 1 second)")
		}
		if (offset < -recvWindow / 1000) {
			Verbose().error("Your computer's clock is too far into the past for Binance Futures (< ", recvWindow / 1000, " seconds)")
		}
	}
	...
	return Object.assign(
		...
		Exchange(state),
		{
			...
			exchangeTime: time,
			exchangeTrade: trade,
			getExchangeStorageAlias: () => "BINANCE",
		}
	)
}
