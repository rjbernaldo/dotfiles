"use strict"
window.DELTA_SYMBOLS = {}
window.DELTA_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function Delta() {
	let state = {
		...
		endpoint: "https://api.delta.exchange",
		fields: {
			public: {
				label: "API Key",
				message: "",
		...
		website: "https://www.delta.exchange/referral/?code=GZDCXW",
	}
	...
	function* symbolInfo(symbol) {
		...
		if (window.DELTA_SYMBOLS_UPDATED < expired) {
			...
			products.forEach((product) => {
				...
				window.DELTA_SYMBOLS[key] = product
				window.DELTA_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (!window.DELTA_SYMBOLS.hasOwnProperty(symbol)) {
			throw new Error("Symbol not found: " + symbol)
		}

		return window.DELTA_SYMBOLS[symbol]
	}
