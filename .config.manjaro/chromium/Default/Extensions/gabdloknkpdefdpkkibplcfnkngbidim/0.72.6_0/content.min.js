...
!function(e,n){function t(e){var n,t,o,i=$(e),a=i,d=v.TORRENT_URL_ATTRIBUTE,l=new RegExp(v.TORRENT_REGEX);if(g(l),a.attr(d)||(a=i.parent("a")),a.attr(d)||(a=i.children("a")),a.length)return o="href"===d?a[0].href:a.attr(d),o&&(n=o.match(l)),g(l,o,n),n&&(t=n.input),t}function o(e,n){var o=t(e.target);o&&(g("Extrated torrent_url: `"+o+"`"),stopEvent(e),n?communicator.sendMessage({method:"addlink-todeluge:withoptions",url:o,domain:v.DOMAIN},s):communicator.sendMessage({method:"storage-get-default_label"},function(e){var n={method:"addlink-todeluge",url:o,domain:v.DOMAIN};e&&e.value&&(n.plugins={Label:e.value}),communicator.sendMessage(n)}))}function i(e){p=!!e.ctrlKey}function a(){p=!1}function d(e){g("RIGHT CLICK","CTRL:",p),p&&o(e,!0)}function l(e){g("LEFT CLICK","CTRL:",p),o(e,p)}function r(){n.webkitHidden||"prerender"==n.webkitVisibilityState||m()}function c(){var e="delugesiphon-modal-"+chrome.runtime.id,n=$("#"+e);n.length||(n=$("<div/>",{id:e})),$("body").append(n),n.on("submit","form",function(e){e.preventDefault();var t=$(this).serializeObject();communicator.sendMessage($.extend({method:"addlink-todeluge",domain:v.DOMAIN},t)),$.modal.close(),n.removeClass("displayed").html("")}),n.on("change",'input[name="options[move_completed]"]',function(){n.find('input[name="options[move_completed_path]"]').prop("disabled",!$(this).is(":checked"))}),n.on("change","input[type=checkbox]",function(){var e=$(this),t=e.attr("name");e.is(":checked")?n.find('form input[name="'+t+'"][type="hidden"]').remove():n.find("form").append($("<input/>",{type:"hidden",name:t,value:""}))}),n.on("click","button[name=cancel]",function(e){e.preventDefault(),$.modal.close(),n.removeClass("displayed").html("")})}function s(e){g("Show Modal",e);var n="delugesiphon-modal-"+chrome.runtime.id,t=Math.max.apply(null,$.map($("body *"),function(e){if("static"!=$(e).css("position"))return parseInt($(e).css("z-index"))||1}));$("#"+n).addClass("displayed").html(f.render($.extend({},e))).modal({blockerClass:n}).parents(".jquery-modal.blocker").css("z-index",(t||1)+10)}function u(){communicator.sendMessage({method:"storage-get-enable_keyboard_macro"},function(e){e.value?(registerEventListener("keydown",i),registerEventListener("keyup",a),registerEventListener("contextmenu",d)):(n.removeEventListener("keydown",i),n.removeEventListener("keyup",a),n.removeEventListener("contextmenu",d))}),communicator.sendMessage({method:"storage-get-enable_leftclick"},function(e){e.value?registerEventListener("click",l,n.body):n.body.removeEventListener("click",l)})}function m(){g("<<<<< SITE INIT >>>>>"),communicator.sendMessage({method:"storage-get-link_regex"},function(e){g("GET LINK REGEX",e.value),e.value&&(v.TORRENT_REGEX=e.value),u()},function(){n.removeEventListener("keydown",i),n.removeEventListener("keyup",a),n.removeEventListener("contextmenu",d),n.body.removeEventListener("click",l),console.error("Background page went away")})}if(n&&n.addEventListener&&n.body&&n.body.addEventListener){var p=!1,v={DOMAIN:e.location.host,TORRENT_REGEX:"^magnet:|(\\/|^)(torrent|torrents)(?=.*action=download)|(\\/|^)(index|download)(\\.php)?(\\&|\\?|\\/)(?=.*torrent)|\\.torrent",TORRENT_URL_ATTRIBUTE:"href",INSTALLED:!1},g=function(){},f=$.templates('<form action="javascript:void(0);"><h3> {{:info.name}} </h3><div class="note"> {{>url}} </div><input type="hidden" value="{{>url}}" name="url"/><div><label for="download_location">download to:</label><input type="text" value="{{>config.download_location}}" name="options[download_location]"></div><div><label for="move_completed">move completed:</label><input type="checkbox" {{if config.move_completed}}checked="checked"{{/if}} value="true" name="options[move_completed]"> <input {{if !config.move_completed }}disabled="disabled"{{/if}} type="text" value="{{>config.move_completed_path}}" name="options[move_completed_path]"></div><div><label for="add_paused">add paused:</label><input type="checkbox" {{if config.add_paused}}checked="checked"{{/if}} value="true" name="options[add_paused]"></div>{{if plugins.Label && plugins.Label.length}}<div class="plugin"><label for="label">label:</label><select name="plugins[Label]"><option value="">-----</option>{{for plugins.Label}}<option value="{{>#data}}">{{>#data}}</option>{{/for}}</select></div>{{/if}}<div class="buttons"><input type="submit" value="Add" name="submit"/> <button name="cancel">Cancel</button></div></form>');communicator.observeConnect(function(){communicator.sendMessage({method:"storage-get-connections"},function(t){for(var o=t.value||[],i=new URL(e.location.href),a=i.pathname.replace(/\/$/,""),d=0,l=o.length;d<l;d++){var u={pathname:""};try{u=new URL(o[d].url)}catch(e){}var p=u.pathname.replace(/\/$/,"");if(i.hostname==u.hostname&&a==p)return void g("[delugesiphon] not initializing on web ui page")}communicator.sendMessage({method:"storage-get-enable_debug_logging"},function(e){e.value&&(g=function(){console.log.apply(console,["[delugesiphon]","["+n.URL+"]"].concat(Array.prototype.slice.call(arguments)))})("Debug logging enabled"),registerEventListener("webkitvisibilitychange",r),m(),c(),communicator.observeMessage(function(e){g("RECV CONTENT MSG",e),"add_dialog"===e.method&&s(e)}),g("INITIALIZED delugesiphon ["+chrome.runtime.id+"]")})})}).observeDisconnect(function(){g("[delugesiphon] Lost connection to background page (probably because it was reloaded). Please refresh this page.")}).init(!!chrome.runtime.id)}}(window,document);
//# sourceMappingURL=maps/content.min.js.map
