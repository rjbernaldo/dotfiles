"use strict"
window.BYBIT_LEVERAGE = {}
window.BYBIT_LISTENER = window.BYBIT_LISTENER || false
window.BYBIT_SYMBOLS = {}
window.BYBIT_SYMBOLS_UPDATED = 0
...
function ByBit() {
	...
	let state = {
		...
		website: "https://www.bybit.com/app/register?ref=Y5739",
	}
	...
	function* setLeverage(symbol, leverage) {
		if (window.BYBIT_LEVERAGE[state.account] === leverage) {
			return true
		}

		window.BYBIT_LEVERAGE[state.account] = leverage
	...
	function* symbolInfo(symbol) {
		...
		if (window.BYBIT_SYMBOLS_UPDATED < expired) {
			const response = yield* get.call(this, "/v2/public/symbols")
			response.forEach((info) => {
				info.symbol = info.name

				window.BYBIT_SYMBOLS[info.name] = info
				window.BYBIT_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (symbol) {
			if (!window.BYBIT_SYMBOLS.hasOwnProperty(symbol)) {
				throw new Error("Unknown market symbol: " + symbol)
			}
			return window.BYBIT_SYMBOLS[symbol]
		}

		return window.BYBIT_SYMBOLS
	}
