	...
	return {
		closeTradingViewListener: function* () {
			if (state.listenerStatus !== TRADINGVIEW_LISTENER_UNSENT) {
				if (state.eventSource) {
					...
					Verbose().info("TradingView Listener closed.")
				}
		...
		openTradingViewListener: function* () {
			if (state.listenerStatus === TRADINGVIEW_LISTENER_UNSENT) {
				...
				const _ = Date.now()
				const lastEventId = yield* this.getTradingViewAttribute("EVENT_ID")
				...
				state.eventSource.onmessage = (e) => {
					state.listenerStatus = TRADINGVIEW_LISTENER_DONE
				...
				state.eventSource.onopen = () => {
					state.listenerStatus = TRADINGVIEW_LISTENER_LOADING
				}

				Verbose().info("TradingView Listener initialized.")
			}
		},
			...
			try {
				...
				if (error instanceof TradingViewListenerError) {
					if (state.lastListenerStatus !== state.listenerStatus) {
						...
						Verbose().warn("TradingView Listener error", error.message)
					}
				} else {
					throw error
				}
