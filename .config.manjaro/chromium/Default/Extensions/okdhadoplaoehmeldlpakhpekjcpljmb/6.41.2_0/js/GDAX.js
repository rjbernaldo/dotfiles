...
function GDAX() {
	...
	function* symbolInfo(symbol) {
		...
		if (window.GDAX_SYMBOLS_UPDATED < expired) {
			const response = yield* get.call(this, "/products")
			response.forEach((info) => {
				window.GDAX_SYMBOLS[info.id] = info
				window.GDAX_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (!window.GDAX_SYMBOLS.hasOwnProperty(symbol)) {
			throw new Error("Unknown market symbol: " + symbol)
		}

		return window.GDAX_SYMBOLS[symbol]
	}
	...
	function* trade(Command) {
		...
		const response = yield* post.call(this, "/orders", params, true)
