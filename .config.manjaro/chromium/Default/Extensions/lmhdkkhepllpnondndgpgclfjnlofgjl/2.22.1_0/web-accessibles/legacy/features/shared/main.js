(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(["core-js/modules/es6.regexp.split"], factory);
  } else if (typeof exports !== "undefined") {
    factory(require("core-js/modules/es6.regexp.split"));
  } else {
    var mod = {
      exports: {}
    };
  ...
  ynabToolKit.shared = function () {
    var storageKeyPrefix = 'ynab-toolkit-';
    return {
      parseSelectedMonth() {
      ...
      showNewReleaseModal() {
        var _ynabToolKit = ynabToolKit,
            assets = _ynabToolKit.assets,
            environment = _ynabToolKit.environment,
            name = _ynabToolKit.name,
            version = _ynabToolKit.version; // beta concatenates the TRAVIS_BUILD_NUMBER so we do this to strip it for
        // the URL that points to diffs on master for beta/development builds

        var githubVersion = version.split('.').slice(0, 3).join('.');
        var githubIssuesLink = '<a href="https://github.com/toolkit-for-ynab/toolkit-for-ynab/issues" target="_blank">GitHub Issues</a>';
        var releaseNotes = ynabToolKit.environment === 'production' ? 'View the <a href="https://github.com/toolkit-for-ynab/toolkit-for-ynab/releases" target="_blank">release notes</a>.' : `<br><br><div class="message">(Release notes are currently only available for production releases. However,
        ...
        var $modal = $(`<div class="toolkit-modal">
      ...
      getToolkitStorageKey(key) {
        var value = localStorage.getItem(storageKeyPrefix + key);
  ...
  (function poll() {
    function isYNABReady() {
      return typeof Ember !== 'undefined' && typeof $ !== 'undefined' && !$('.ember-view.is-loading').length && typeof ynabToolKit !== 'undefined' && typeof YNABFEATURES !== 'undefined';
    }

    if (isYNABReady()) {
      ynabToolKit.pageReady = true;
      var latestVersionKey = `latest-version-${ynabToolKit.environment}`;
      var latestVersion = ynabToolKit.shared.getToolkitStorageKey(latestVersionKey);
      ...
      var deprecatedLatestVersion = ynabToolKit.shared.getToolkitStorageKey('latest-version');
