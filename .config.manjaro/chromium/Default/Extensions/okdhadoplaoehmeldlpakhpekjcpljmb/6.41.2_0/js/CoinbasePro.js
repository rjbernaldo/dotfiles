...
function CoinbasePro() {
	...
	function* symbolInfo(symbol) {
		...
		if (window.COINBASE_PRO_UPDATED < expired) {
			...
			response.forEach((info) => {
				...
				window.COINBASE_PRO[info.id] = info
				window.COINBASE_PRO_UPDATED = Date.now()
			})
		}

		if (symbol) {
			if (!window.COINBASE_PRO.hasOwnProperty(symbol)) {
				throw new Error("Unknown market symbol: " + symbol)
			}
			return window.COINBASE_PRO[symbol]
		}

		return window.COINBASE_PRO
	}
