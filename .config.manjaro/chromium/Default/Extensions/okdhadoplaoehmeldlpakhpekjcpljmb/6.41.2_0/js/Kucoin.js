...
window.KUCOIN_TICKERS = {}
window.KUCOIN_TICKERS_UPDATED = 0
window.KUCOIN_SYMBOLS = {}
window.KUCOIN_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function Kucoin() {
	...
	function* symbolInfo(symbol) {
		...
		if (window.KUCOIN_SYMBOLS_UPDATED < expired) {
			...
			response.forEach((info) => {
				...
				window.KUCOIN_SYMBOLS[info.symbol] = info
				window.KUCOIN_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (symbol) {
			if (!window.KUCOIN_SYMBOLS.hasOwnProperty(symbol)) {
				throw new Error("Unknown market symbol: " + symbol)
			}
			return window.KUCOIN_SYMBOLS[symbol]
		}

		return window.KUCOIN_SYMBOLS
	}

	function* symbolTicker(symbol) {
		const expired = Date.now() - 60000 // 1 minute
		if (window.KUCOIN_TICKERS_UPDATED < expired) {
			const response = yield* get.call(this, "/api/v1/market/allTickers")
			window.KUCOIN_TICKERS_UPDATED = response.time
			response.ticker.forEach((ticker) => {
				window.KUCOIN_TICKERS[ticker.symbol] = ticker
			})
		}

		if (symbol) {
			if (!window.KUCOIN_TICKERS.hasOwnProperty(symbol)) {
				throw new Error("Unknown ticker symbol: " + symbol)
			}
			return window.KUCOIN_TICKERS[symbol]
		}

		return window.KUCOIN_TICKERS
	}
