"use strict"
window.DERIBIT_NONCE = 0
window.DERIBIT_NONCE_OFFSET = -1
window.DERIBIT_SYMBOLS = {}
window.DERIBIT_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function Deribit() {
	let state = {
		...
		website: "https://www.deribit.com/reg-3466.8566?q=home",
	}
	...
	function* getNonce() {
		...
		if (window.DERIBIT_NONCE != nonce) {
			window.DERIBIT_NONCE_OFFSET = -1
		}

		window.DERIBIT_NONCE = nonce
		window.DERIBIT_NONCE_OFFSET++

		nonce += window.DERIBIT_NONCE_OFFSET
	...
	function* symbolInfo(symbol) {
		...
		if (window.DERIBIT_SYMBOLS_UPDATED < expired) {
			...
			for (let i = 0; i < currencies.length; i++) {
				...
				instruments.forEach((instrument) => {
					...
					window.DERIBIT_SYMBOLS[instrument.instrument_name] = instrument
					window.DERIBIT_SYMBOLS_UPDATED = Date.now()
				})
			}
		}

		if (!window.DERIBIT_SYMBOLS.hasOwnProperty(symbol)) {
			throw new Error("Symbol not supported: " + symbol)
		}

		return window.DERIBIT_SYMBOLS[symbol]
	}
