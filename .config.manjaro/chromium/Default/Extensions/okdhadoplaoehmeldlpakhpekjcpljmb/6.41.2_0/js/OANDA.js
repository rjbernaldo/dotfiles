"use strict"
window.OANDA_SYMBOLS = {}
window.OANDA_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function OANDA() {
	...
	function* instrument(instrument) {
		...
		if (window.OANDA_SYMBOLS_UPDATED < expired) {
			const response = yield* get.call(this, "/v3/accounts/{accountID}/instruments")
			response.instruments.forEach((Instrument) => {
				window.OANDA_SYMBOLS[Instrument.name] = Instrument
				window.OANDA_SYMBOLS_UPDATED = Date.now()
			})
		}

		for (const name in window.OANDA_SYMBOLS) {
			if (window.OANDA_SYMBOLS.hasOwnProperty(name)) {
				const Instrument = window.OANDA_SYMBOLS[name]
		...
		if (!window.OANDA_SYMBOLS.hasOwnProperty(instrument)) {
			throw new Error("Instrument not supported: " + instrument)
		}

		return window.OANDA_SYMBOLS[instrument]
	}
