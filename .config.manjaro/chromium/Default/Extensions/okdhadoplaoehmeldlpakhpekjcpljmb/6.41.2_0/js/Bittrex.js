...
window.BITTREX_SYMBOLS = {}
window.BITTREX_SYMBOLS_UPDATED = 0
/**
 *
 * @returns {*}
 * @constructor
...
function Bittrex() {
	...
	function* ordersCancelAll(Command) {
		const pair = yield* symbolPair.call(this, Command.s)
		let types = []
		if (Command.isBid) {
	...
	function* symbolInfo(symbol) {
		...
		if (window.BITTREX_SYMBOLS_UPDATED < expired) {
			const response = yield* get.call(this, "/public/getmarkets")
			response.forEach((market) => {
				window.BITTREX_SYMBOLS[market.MarketName] = market
				window.BITTREX_SYMBOLS_UPDATED = Date.now()
			})
		}

		if (symbol) {
			if (!window.BITTREX_SYMBOLS.hasOwnProperty(symbol)) {
				throw new Error("Symbol not supported: " + symbol)
			}

			return window.BITTREX_SYMBOLS[symbol]
		}
	...
	function* symbolPair(symbol) {
		...
		const baseCurrencies = Object.values(yield* symbolInfo.call(this) || {})
			.filter(market => market.hasOwnProperty("BaseCurrency"))
			.map(market => market.BaseCurrency)
		const baseCurrencySet = new Set(baseCurrencies) // unique values
		const currencies = [...baseCurrencySet].join("|")

		// Bittrex URL (e.g. https://bittrex.com/Market/Index?MarketName=BTC-PINK)
		reg = new RegExp('^(' + currencies + ')-?(.*?)$', 'i')
	...
	function* trade(Command) {
		...
		const pricePrecision = pair.main === "USD" ? 3 : 8
		...
		let params = {}
		params.quantity = Command.q.resolve(pair.precision)
